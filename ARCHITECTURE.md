...
`BASE_CAUSE_ALIASES` / `BASE_PRODUCT_ALIASES`는 **값 전체가 정확히 일치할 때만** 치환한다.
부분 치환은 `"데일리라이트"` → `"데일리데일리라이트"` 같은 사고를 낸다 (구 `normalizeProductName`의 버그).

```
스펀지접힘 → 스펀지 접힘      구멍·바늘구멍 → 원단구멍
사이즈미달·사이즈이단·미달 → 사이즈 미달      라이트 → 데일리라이트      데일리 키즈 → 키즈
```
